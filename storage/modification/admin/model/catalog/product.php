...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
		...
		$this->model_extension_module_product_status->addProductStatuses($product_id, $data);
		...
		foreach ($data['product_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "product_description SET product_id = '" . (int)$product_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', description = '" . $this->db->escape($value['description']) . "', tag = '" . $this->db->escape($value['tag']) . "', meta_title = '" . $this->db->escape($value['meta_title']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "'");
				...
				if(isset($value['default_meta'])) {
					$this->db->query("UPDATE " . DB_PREFIX . "product_description SET default_meta = '".(int)$value['default_meta']."' WHERE product_id = '" . (int)$product_id . "' AND language_id = '" . (int)$language_id . "'");
				}
			
		}


		$this->db->query("DELETE FROM " . DB_PREFIX . "seo_faq WHERE product_id = '" . (int)$product_id . "' AND type = 'product'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "seo_faq_description WHERE product_id = '" . (int)$product_id . "' AND type = 'product'");
		if (isset($data['faq'])) {
			foreach ($data['faq'] as $product_faq) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "seo_faq SET product_id = '" . (int)$product_id . "', `sort_order` = '" . (int)$product_faq['sort_order'] . "', type = 'product'");
				$faq_id = $this->db->getLastId(); 
				foreach ($product_faq['faq_data'] as $language_id => $value) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "seo_faq_description SET faq_id = '" . (int)$faq_id . "', product_id = '" . (int)$product_id . "', language_id = '" . (int)$language_id . "', question = '" . $this->db->escape($value['question']) . "', answer = '" . $this->db->escape($value['answer']) . "', type = 'product'");
				}
			}
		} 
...
if (isset($data['partscount_pp'])) {
	if ($data['partscount_pp'] || $data['partscount_ii'] || $data['partscount_mb'] || $data['product_pp'] || $data['product_ii'] || $data['product_mb'] || $data['markup_pp'] || $data['markup_ii'] || $data['markup_mb']) {
        ...
        if (isset($ocdw_in_stock_alert_form_data['activate']) && $ocdw_in_stock_alert_form_data['activate']) {
          if (in_array($ocdw_in_stock_alert_form_data['notification_event'],[1,3])) {
            $this->load->model('extension/ocdevwizard/in_stock_alert');

            $ocdw_in_stock_alert_filter_data = [
              'product_id'     => $product_id,
              'quantity'       => $data['quantity'],
              'product_option' => (isset($data['product_option'])) ? $data['product_option'] : [],
              'form_data'      => $ocdw_in_stock_alert_form_data
            ];

            $this->model_extension_ocdevwizard_in_stock_alert->notifyRecord($ocdw_in_stock_alert_filter_data);
          }
        }
	...
	public function copyProduct($product_id) {
		...
		$this->load->model('extension/module/product_status');
                $data['product_status'] = $this->model_extension_module_product_status->getProductStatuses($product_id);
	...
	public function deleteProduct($product_id) {
		...
		$this->load->model('extension/module/product_status');
		$this->model_extension_module_product_status->deleteProductStatuses($product_id);
		
      

		
$this->load->model('extension/module/promotion');
$this->model_extension_module_promotion->deletePromotionProduct($product_id);
        ...
        if (isset($ocdw_in_stock_alert_form_data['activate']) && $ocdw_in_stock_alert_form_data['activate']) {
          $this->load->model('extension/ocdevwizard/in_stock_alert');
          $this->model_extension_ocdevwizard_in_stock_alert->deleteRecord($product_id,1);
        }
        // end: OCdevWizard In Stock Alert
...
'p2c.category_id',
	...
	public function getTotalProductsByProfileId($recurring_id) {
		...
		public function getProductFaq($product_id) {
			$faq = [];
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "seo_faq WHERE product_id = '" . (int)$product_id . "' AND type = 'product' ORDER BY sort_order ASC");
