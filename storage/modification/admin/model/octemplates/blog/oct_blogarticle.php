...
class ModelOCTemplatesBlogOCTBlogArticle extends Model {
	public function addArticle($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "oct_blogarticle SET date_available = '" . $this->db->escape($data['date_available']) . "', status = '" . (int)$data['status'] . "', sort_order = '" . (int)$data['sort_order'] . "', blogauthor_id = '" . (int)$data['blogauthor_id'] . "', date_added = '" . $this->db->escape($data['date_added']) . "', date_modified = NOW()");
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "seo_faq WHERE category_id = '" . (int)$blogarticle_id . "' AND type = 'oct_blog_article'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "seo_faq_description WHERE category_id = '" . (int)$blogarticle_id . "' AND type = 'oct_blog_article'");
		if (isset($data['faq'])) {
			foreach ($data['faq'] as $information_faq) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "seo_faq SET category_id = '" . (int)$blogarticle_id . "', `sort_order` = '" . (int)$information_faq['sort_order'] . "', type = 'oct_blog_article'");
				$faq_id = $this->db->getLastId(); 
				foreach ($information_faq['faq_data'] as $language_id => $value) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "seo_faq_description SET faq_id = '" . (int)$faq_id . "', category_id = '" . (int)$blogarticle_id . "', language_id = '" . (int)$language_id . "', question = '" . $this->db->escape($value['question']) . "', answer = '" . $this->db->escape($value['answer']) . "', type = 'oct_blog_article'");
				}
			}
		} 
		...
		if (isset($data['author'])) {
				...
				$this->db->query("INSERT INTO " . DB_PREFIX . "oct_blogarticle_to_author SET blogarticle_id = '" . (int)$blogarticle_id . "', blogauthor_id = '" . (int)$data['blogauthor_id'] . "'");
	...
	public function editArticle($blogarticle_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "oct_blogarticle SET date_available = '" . $this->db->escape($data['date_available']) . "', date_added = '" . $this->db->escape($data['date_added']) . "', status = '" . (int)$data['status'] . "', sort_order = '" . (int)$data['sort_order'] . "', blogauthor_id = '" . (int)$data['blogauthor_id'] . "', date_modified = NOW() WHERE blogarticle_id = '" . (int)$blogarticle_id . "'");
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "oct_blogarticle_to_author WHERE blogarticle_id = '" . (int)$blogarticle_id . "'");
	...
	public function getArticlesByAuthorId($blogauthor_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "oct_blogarticle p LEFT JOIN " . DB_PREFIX . "oct_blogarticle_description pd ON (p.blogarticle_id = pd.blogarticle_id) LEFT JOIN " . DB_PREFIX . "oct_blogarticle_to_author p2c ON (p.blogarticle_id = p2c.blogarticle_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p2c.blogauthor_id = '" . (int)$blogauthor_id . "' ORDER BY pd.name ASC");
	...
	public function getArticleCategories($blogarticle_id) {
		...
		public function getArticleAuthor($blogarticle_id) {
		$article_author_data = array();

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "oct_blogarticle_to_author WHERE blogarticle_id = '" . (int)$blogarticle_id . "'");

		foreach ($query->rows as $result) {
			$article_author_data[] = $result['blogauthor_id'];
		}
	...
	public function getArticleImages($blogarticle_id) {
		...
		public function getBlogArticleFaq($blogarticle_id) {
			$faq = [];
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "seo_faq WHERE category_id = '" . (int)$blogarticle_id . "' AND type = 'oct_blog_article' ORDER BY sort_order ASC");
