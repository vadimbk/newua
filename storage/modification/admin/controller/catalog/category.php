...
class ControllerCatalogCategory extends Controller {
    ...
    // SEO URL Generator PRO . begin
      	...
      	foreach ($products as $product) {
			...
			if ($related_info) {
				$data['product_relateds'][] = array(
					...
					'name'       => $related_info['sku'] . " <=> " . $related_info['name']
				);
			}
		}
		...
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');
  // ==== Related blog articles (OCT Blog) ====
$data['oct_blogarticles'] = [];

// Ukrainian comment from developers below
$this->load->model('catalog/category'); // на всяк випадок (він уже завантажений)

if (isset($this->request->post['oct_blogarticle'])) {
    $article_ids = $this->request->post['oct_blogarticle'];
} elseif (isset($this->request->get['category_id'])) {
    $article_ids = $this->model_catalog_category->getCategoryBlogArticles((int)$this->request->get['category_id']);
} else {
    $article_ids = [];
}

$data['oct_blogarticles'] = $this->model_catalog_category->getOctBlogarticlesByIds($article_ids);
...
// =========================================
	...
	protected function validateForm() {
		...
		foreach ($this->request->post['category_description'] as $language_id => $value) {
			...
			if (((utf8_strlen($value['meta_title']) < 1) || (utf8_strlen($value['meta_title']) > 255)) && !isset($oct_ultrastore_seo_title_data['category']['title_empty'])) {
	...
	public function relatedAutocomplete(){
		 $json=array();
		 ...
		 if(isset($this->request->get['filter_name'])||isset($this->request->get['filter_model'])||isset($this->request->get['filter_category_id'])){
			 ...
			 foreach($results as $result){
				 $json[]=array(
					 ...
					 'name'=>$result['sku'] . " <=> " . strip_tags(html_entity_decode($result['name'],ENT_QUOTES,'UTF-8')),
					 'model'=>$result['model'],
					 'price'=>$result['price']
				 );
			 }
...
public function blogArticleAutocomplete() {
    $json = [];

    if (!empty($this->request->get['filter_name'])) {
        $this->load->model('catalog/category');

$results = $this->model_catalog_category->getOctBlogarticlesAutocomplete(
