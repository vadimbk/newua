...
'category'   => $category,
				...
				'price'      => $this->currency->format($result['price'], 'USD'),
				'special_ua'    => $this->currency->format($special, 'UAH'),//$this->config->get('config_currency'),
				'price_ua'      => $this->currency->format($result['price'], 'UAH'),//$this->config->get('config_currency'),
				'special'    => $special,
				/*'special'    => $this->currency->format($special, 'USD'),*/
...
$data['sort_category'] = $this->url->link('catalog/product', 'user_token=' . $this->session->data['user_token'] . '&sort=p2c.category' . $url, true);
		$data['sort_name'] = $this->url->link('catalog/product', 'user_token=' . $this->session->data['user_token'] . '&sort=pd.name' . $url, true);
                ...
                $data['sort_manufacturer'] = $this->url->link('catalog/product', 'user_token=' . $this->session->data['user_token'] . '&sort=p.manufacturer_id' . $url, true);
		...
		$data['sort_upc'] = $this->url->link('catalog/product', 'user_token=' . $this->session->data['user_token'] . '&sort=p.upc' . $url, true);
		$data['sort_price'] = $this->url->link('catalog/product', 'user_token=' . $this->session->data['user_token'] . '&sort=p.price' . $url, true);
		$data['sort_price_ua'] = $this->url->link('catalog/product', 'user_token=' . $this->session->data['user_token'] . '&sort=p.price_ua' . $url, true);
...
$data['filter_category'] = $filter_category;
    ...
    // OCFilter end
	  ...
	  $this->document->addScript('view/javascript/dev-os/attribute_ext.js');
    ...
    /* Bulk Specials Editor */
			...
			$data['oct_ultrastore_seo_title_data'] = $this->config->get('theme_oct_ultrastore_seo_title_data');
    ...
    // OCFilter end
		...
		if (isset($this->error['sku'])) {
			...
			$data['error_sku'] = '';
				...
				if (isset($this->error['upc'])) {
			$data['error_upc'] = $this->error['upc'];
		} else {
			$data['error_upc'] = '';
		}
...
$data['sku'] = $autosku_query1->row["MAX(sku)"]+1;
		}
      ...
      //begin_devos_attribute_ext
	  $data['dae_product_id'] =  (isset($this->request->get['product_id']))?(int)$this->request->get['product_id']:0;
      //end_devos_attribute_ext
		...
		if (isset($this->request->post['product_seo_url'])) {
 		...
 		if (isset($this->request->post['faq'])) {
			...
			$faq = $this->model_catalog_product->getProductFaq($this->request->get['product_id']);
		} else {
			$faq = [];
		}
		$data['product_faq'] = [];
		foreach ($faq as $product_faq) {
			$data['product_faq'][] = [
				'faq_data' => $product_faq['faq_data'],
		...
		$this->load->language('extension/module/product_status_product');
		...
		if (isset($this->request->get['product_id'])) {
			$data['product_statuses'] = $this->model_extension_module_product_status->getProductStatuses($this->request->get['product_id']);
		} else {
	...
	protected function validateForm() {
		...
		foreach ($this->request->post['product_description'] as $language_id => $value) {
			...
			$oct_ultrastore_seo_title_data = $this->config->get('theme_oct_ultrastore_seo_title_data');
			
			if (((utf8_strlen($value['meta_title']) < 1) || (utf8_strlen($value['meta_title']) > 255)) && !isset($oct_ultrastore_seo_title_data['product']['title_empty'])) {
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_model'])|| isset($this->request->get['filter_sku'])|| isset($this->request->get['filter_upc'])) {
			$this->load->model('catalog/product');
			$this->load->model('catalog/option');
