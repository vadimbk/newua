...
$this->document->addScript('view/javascript/ckeditor/summernote_replacer.js');
    ...
    if (isset($this->request->get['ocfilter_page_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
      ...
      $this->info = $page_info;
		...
		$data['languages'] = $this->model_localisation_language->getLanguages();
				...
				$video_info = $this->model_extension_ocfilter_page->getPageVideo($this->request->get['ocfilter_page_id']);	
				//echo ($this->request->get['ocfilter_page_id']);
				//echo $video_info;
				
				if (isset($this->request->post['video'])) {
				$data['video'] = $this->request->post['video'];
				}
				else if (!empty($video_info)) {
				$data['video'] = $this->model_extension_ocfilter_page->getPageVideo($this->request->get['ocfilter_page_id']);
				} else {
				$data['video'] = '';
				}				
