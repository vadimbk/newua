...
protected function wm_translit($text) {
  ...
  return trim(strip_tags(str_replace(array_keys($disallow_symbols), array_values($disallow_symbols), trim(html_entity_decode($text, ENT_QUOTES, 'UTF-8')))), '-');
	...
	public function index() {
		...
		if ($this->config->get('imagemanager_status')) {
			if (isset($this->request->get['multiple'])) {
				$this->load->controller('extension/module/imagemanager/showfmmulti');
			}
			else {
				$this->load->controller('extension/module/imagemanager/showfm');
			}
			return;
		}
...
if ($this->config->get('config_enable_ext_allowed_files') && isset($this->request->get['cke']) && $this->request->get['cke'] == 'info:url') {
  $allowed = array();
...
if ($this->config->get('config_enable_ext_allowed_files')) {
  // Allowed file extension types
  $allowed = array();
