...
class ControllerCommonColumnLeft extends Controller {
	public function index() {
		if (isset($this->request->get['user_token']) && isset($this->session->data['user_token']) && ($this->request->get['user_token'] == $this->session->data['user_token'])) {
			...
			if ($this->user->hasPermission('access', 'extension/theme/oct_ultrastore')) {
				$oct_ultrastore[] = array(
					'name'	   => $this->language->get('text_oct_ultrastore'),
			...
			if ($this->user->hasPermission('access', 'octemplates/blog/oct_blogarticle')) {
				$blogs[] = [
					...
					'children' => []
						...
						if ($this->user->hasPermission('access', 'octemplates/blog/oct_blogauthor')) {
				$blogs[] = [
					'name'	   => $this->language->get('text_oct_blogauthor'),
					'href'     => $this->url->link('octemplates/blog/oct_blogauthor', 'user_token=' . $this->session->data['user_token'], true),
					'children' => []
				];
			}
    ...
    if ($this->user->hasPermission('access', 'extension/module/timer')) {        
        $catalog[] = array(
            'name'     => '<b>'. $this->language->get('SPECIALS EDITOR') . '</b>',
      ...
      //begin_devos_attribute_ext
		$attribute[] = array(
			'name'     => 'Attribute Ext',
			'href'     => $this->url->link('catalog/devos_attribute_ext/getListAttributes', 'user_token=' . $this->session->data['user_token'], true),
			'children' => array()	
		);
      //end_devos_attribute_ext
        ...
        if ($this->user->hasPermission('access', 'extension/ocdevwizard/helper')) {
          if (version_compare(VERSION, '3.0.0.0', '>=')) {
            $data['menus'][] = [
              'id'       => 'menu-ocdevwizard',
              'icon'     => 'fa fa-lightbulb-o fa-fw',
              'name'     => 'OCdevWizard',
              'href'     => $this->url->link('extension/ocdevwizard/helper', 'user_token='.$this->session->data['user_token'], true),
              'children' => []
            ];
          } else {
            $data['menus'][] = [
              ...
              'icon'     => 'fa fa-lightbulb-o fa-fw',
              'name'     => 'OCdevWizard',
              'href'     => $this->url->link('extension/ocdevwizard/helper', 'token='.$this->session->data['token'], true),
              'children' => []
            ];
          }
        }
      ...
      if ($this->user->hasPermission('access', 'extension/payment/liqpayplus') || $this->user->hasPermission('access', 'extension/payment/liqpayplus_card')) {
        $sale[] = array(
          'name'     => 'LiqPay',
          'href'     => $this->url->link('extension/payment/liqpayplus/status', 'user_token=' . $this->session->data['user_token'], true),
          'children' => array()   
        );  
      }
			// Voucher
