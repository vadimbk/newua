...
class ControllerCommonHeader extends Controller {
	public function index() {
        ...
        if ($this->user->hasPermission('access', 'extension/ocdevwizard/in_stock_alert')) {
          ...
          $this->model_extension_ocdevwizard_in_stock_alert->createDBTables();
          $data['ocdw_in_stock_alert_form_data'] = $ocdw_in_stock_alert_form_data = (array)$this->model_extension_ocdevwizard_helper->getSettingData('in_stock_alert_form_data', (int)$this->config->get('config_store_id'));
        }
        // end: OCdevWizard In Stock Alert
      

//CKEditor
if ($this->config->has('config_editor_default') && $this->config->get('config_editor_default')) {
...
if (version_compare(VERSION, '3', '>=')) {
  ...
  $data['ckeditor_token'] = 'token';
		...
		if ($data['scripts']) {
			array_unshift($data['scripts'], 'view/javascript/elfinder/js/summernote-button.js');
		}
		else {
			$data['scripts'][] = 'view/javascript/elfinder/js/summernote-button.js';
		}
