...
class ControllerOCTemplatesModuleOctPopupCart extends Controller {
		...
		if ($this->config->get('theme_oct_ultrastore_popup_cart_status') && isset($this->request->server['HTTP_X_REQUESTED_WITH']) && !empty($this->request->server['HTTP_X_REQUESTED_WITH']) && strtolower($this->request->server['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
            ...
            foreach ($this->cart->getProducts() as $product) {
                if ($product['cart_id'] == $this->request->request['remove']) {
                    $this->session->data['remove_product_id'] = $product['product_id'];
                } 
            }
            ...
            $this->session->data['add_update_cart_id'] = $this->request->request['update'];
    ...
    public function status_cart() {
	    if ($this->config->get('theme_oct_ultrastore_popup_cart_status') && isset($this->request->server['HTTP_X_REQUESTED_WITH']) && !empty($this->request->server['HTTP_X_REQUESTED_WITH']) && strtolower($this->request->server['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
            ...
            // Если после удаления пустая корзина
            $this->load->model('tool/remarketing');
            if ($this->config->get('remarketing_status') && !$this->model_tool_remarketing->isBot() && $this->cart->countProducts() < 1) {
                $product_info = [];
                $json['remarketing'] = $this->model_tool_remarketing->remarketingRemoveFromCart($product_info, $quantity);
            } elseif($this->config->get('remarketing_status') && !$this->model_tool_remarketing->isBot() && isset($this->session->data['remove_product_id'])) {
                $this->load->model('catalog/product');
                $product_info = $this->model_catalog_product->getProduct($this->session->data['remove_product_id']);
                unset($this->session->data['remove_product_id']);
                $json['remarketing'] = $this->model_tool_remarketing->remarketingRemoveFromCart($product_info, $quantity);
            // Иначе считаем что товар добавили, формируем новый
            } elseif($this->config->get('remarketing_status') && !$this->model_tool_remarketing->isBot() && isset($this->session->data['add_update_cart_id'])) {
                $this->load->model('catalog/product');
                $product_info = [];
                foreach ($this->cart->getProducts() as $product) {
                    if ($product['cart_id'] == $this->session->data['add_update_cart_id']) {
                        $product_info = $this->model_catalog_product->getProduct($product['product_id']);
                        $quantity = $product['quantity'];
                    } 
                }
                unset($this->session->data['add_update_cart_id']);
