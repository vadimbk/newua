...
class ControllerOCTemplatesBlogOCTBlogLatest extends Controller {
	public function index() {
		...
		$this->document->setTitle($this->language->get('text_blog_home'));
		$this->document->setDescription('Blog');
		//$this->document->setKeywords('Blog');

		$data['heading_title'] = $this->language->get('text_blog_home');
		...
		foreach ($results as $result) {
			...
			$data['blog_categories'][] = [
				...
				'href' => $this->url->link('octemplates/blog/oct_blogcategory', 'blog_path=' . $result['blogcategory_id'] . $url)
			];
		}
		...
		foreach ($results as $result) {
			if ($result['image']) {
				$image = $this->model_tool_image->resize($result['image'], $oct_blogsettings_data['articles_width'], $oct_blogsettings_data['articles_height'], 'h');
				if($i==0) $image = $this->model_tool_image->resize($result['image'], $oct_blogsettings_data['articles_width'] * 3, $oct_blogsettings_data['articles_height'], 'w');
			} else {
				$image = $this->model_tool_image->resize('placeholder.png', $oct_blogsettings_data['articles_width'], $oct_blogsettings_data['articles_height'], 'h');
				if($i==0) $image = $this->model_tool_image->resize('placeholder.png', $oct_blogsettings_data['articles_width'] * 3, $oct_blogsettings_data['articles_height'], 'w');
			}
		...
		foreach($limits as $value) {
			$data['limits'][] = [
				'text'  => $value,
				'value' => $value,
				'href'  => $this->url->link('octemplates/blog/oct_bloglatest', '' . $url . '&limit=' . $value)
			];
		}
		...
		$pagination->url = $this->url->link('octemplates/blog/oct_bloglatest', '' . $url . '&page={page}');
		...
		if ($page == 1) {
		    $this->document->addLink($this->url->link('octemplates/blog/oct_bloglatest', ''), 'canonical');
		} else {
			$this->document->addLink($this->url->link('octemplates/blog/oct_bloglatest', '' . '&page='. $page), 'canonical');
		}

		if ($page > 1) {
		    $this->document->addLink($this->url->link('octemplates/blog/oct_bloglatest', '' . (($page - 2) ? '&page='. ($page - 1) : '')), 'prev');
		}

		if ($limit && ceil($article_total / $limit) > $page) {
		    $this->document->addLink($this->url->link('octemplates/blog/oct_bloglatest', '' . '&page='. ($page + 1)), 'next');
		}
