...
class ControllerInformationContact extends Controller {
	...
	public function index() {
		...
		// CUSTOM CODE START: Error handling for the challenge
		...
		// CUSTOM CODE START: Sticky form for the challenge field
	...
	protected function validate() {
		...
		// CUSTOM CODE START: Challenge Validation
		// Decoded key is 'geroyam' (in Cyrillic)
		$key = base64_decode('0LPQtdGA0L7Rj9C8'); 
		
		if (empty($this->request->post['ua_challenge'])) {
			$this->error['ua_challenge'] = 'Security check failed.';
		} else {
			// Trim spaces and convert to lowercase UTF-8
			$input = mb_strtolower(trim($this->request->post['ua_challenge']), 'UTF-8');
			
			// Check if the input STARTS with the key (position 0)
			if (mb_strpos($input, $key, 0, 'UTF-8') !== 0) {
				$this->error['ua_challenge'] = 'Security check failed.';
			}
		}
		// CUSTOM CODE END
