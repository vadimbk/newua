	...
	public function index() {
		if ($this->config->get('feed_google_sitemap_status')) {
			...
			if ($this->config->get('oct_blogsettings_status')) {
				...
				$authors = $this->model_octemplates_blog_oct_blogauthor->getauthors();

				foreach ($authors as $author) {
					$output .= '<url>';
					$output .= '  <loc>' . $this->url->link('octemplates/blog/oct_blogauthor', 'blogauthor_id=' . $author['blogauthor_id']) . '</loc>';
					$output .= '  <changefreq>weekly</changefreq>';
					$output .= '  <lastmod>' . date('Y-m-d\TH:i:sP', strtotime($author['date_modified'])) . '</lastmod>';
					$output .= '  <priority>1.0</priority>';

					if ($author['image']) {
						$output .= '  <image:image>';
						$output .= '  <image:loc>' . $this->model_tool_image->resize($author['image'], $oct_blogsettings_data['author_width'], $oct_blogsettings_data['author_height']) . '</image:loc>';
						$output .= '  <image:caption>' . $author['name'] . '</image:caption>';
						$output .= '  <image:title>' . $author['name'] . '</image:title>';
						$output .= '  </image:image>';
					}
