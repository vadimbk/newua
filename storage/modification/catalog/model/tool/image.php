...
if ($extension != 'gif') {
	...
	$image_new = $image_old;
		...
		if ($this->request->server['HTTPS']) {
			return $this->config->get('config_ssl') . 'image/' . ($oct_webp_image ? $oct_webp_image : $image_new);
		} else {
			return $this->config->get('config_url') . 'image/' . ($oct_webp_image ? $oct_webp_image : $image_new);
		}
	}
}
