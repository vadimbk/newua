    ...
    class ControllerSeoLangMovaMova extends Controller {
        ...
        private function seourl($seo_url) {
            ...
            if (isset($langmark_multi['prefix'])) {
                $lm_slash = strpos($langmark_multi['prefix'], '/');
                $host_from_prefix = $lm_slash !== false ? substr($langmark_multi['prefix'], 0, $lm_slash) : '';
                // staging-host fix: prefer current request host when it suffix-matches the prefix host
                $lm_req_host = isset($this->request->server['HTTP_HOST']) ? preg_replace('/:\d+$/', '', $this->request->server['HTTP_HOST']) : '';
                if ($host_from_prefix !== '' && $lm_req_host !== '' && ($lm_req_host === $host_from_prefix || substr($lm_req_host, -strlen('.' . $host_from_prefix)) === '.' . $host_from_prefix)) {
                    $langmark_multi['prefix'] = $lm_req_host . substr($langmark_multi['prefix'], $lm_slash);
                    $host_from_prefix = $lm_req_host;
                }
            } else {
                $host_from_prefix = '';
            }
