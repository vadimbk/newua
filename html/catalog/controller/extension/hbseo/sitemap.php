...
class ControllerExtensionHbseoSitemap extends Controller {
	public function index() {
		if ($this->config->get('hb_sitemap_enable')) {	
			...
			foreach ($languages->rows as $language) {
				...
				if ($this->config->get('hb_sitemap_product')) {
					for ($x = 1; $x <= $number_of_pages; $x++) {
						...
						if ($htaccess_enabled){
							...
							$output .= '<loc>'.$this->url->link('extension/hbseo/sitemap/products', '&hbxmllang='.$language_code.'&page=' . $x).'</loc>';
						}
				...
				if ($this->config->get('hb_sitemap_journal2')) {
					$output .='<sitemap>';
					if ($htaccess_enabled){
						$output .= '<loc>'.$store_url.'sitemaps/'.$language_code.'/journal2blog_sitemap.xml</loc>';
					}else{
						$output .= '<loc>'.$this->url->link('extension/hbseo/sitemap/journal2blog','hbxmllang='.$language_code).'</loc>';
					}
					$output .= '<lastmod>'.date('c',strtotime($this->lastupdatedProduct())).'</lastmod>';
					$output.='</sitemap>';
				}
				if ($this->config->get('hb_sitemap_journal3')) {
					$output .='<sitemap>';
					if ($htaccess_enabled){
						$output .= '<loc>'.$store_url.'sitemaps/'.$language_code.'/journal3blog_sitemap.xml</loc>';
					}else{
						$output .= '<loc>'.$this->url->link('extension/hbseo/sitemap/journal3blog','hbxmllang='.$language_code).'</loc>';
					}
					$output .= '<lastmod>'.date('c',strtotime($this->lastupdatedProduct())).'</lastmod>';
					$output.='</sitemap>';
				}
	...
	public function products() {
		if ($this->config->get('hb_sitemap_product')) {
			...
			foreach ($products as $product) {
				...
				{						
					$language_code='uk';
					$link = str_replace(HTTP_SERVER, '/', $this->url->link('product/product', 'product_id=' . $product['product_id']) );
					$output .= '<loc>' . HTTP_SERVER . $language_code . $link . '</loc>';
				}
	...
	public function brand() {
		if ($this->config->get('hb_sitemap_brand')) {
			...
			foreach ($manufacturers as $manufacturer) {
				...
				{
					$language_code='uk';
					$link = str_replace(HTTP_SERVER, '/', $this->url->link('product/manufacturer/info', 'manufacturer_id=' . $manufacturer['manufacturer_id']));
					$output .= '<loc>' . HTTP_SERVER . $language_code . $link . '</loc>';
	...
	public function information() {
		if ($this->config->get('hb_sitemap_info')) {
			...
			foreach ($informations as $information) {
				...
				{	
				$language_code='uk';
				$link = str_replace(HTTP_SERVER, '/', $this->url->link('information/information', 'information_id=' . $information['information_id']));
				$output .= '<loc>' . HTTP_SERVER . $language_code . $link . '</loc>';
				}
				$output .= '<changefreq>weekly</changefreq>';
	...
	protected function getCategories($parent_id, $current_path = '') {
		...
		foreach ($results as $result) {
			...
			{
				$language_code='uk';
				$link = str_replace(HTTP_SERVER, '/', $this->url->link('product/category', 'path=' . $new_path));
				$output .= '<loc>' . HTTP_SERVER . $language_code . $link . '</loc>';
			}
			$output .= '<changefreq>weekly</changefreq>';
			$output .= '<priority>0.7</priority>';
