...
class ControllerExtensionModuleOCFilter extends Controller {
  use Helper;
	...
	//echo "ocfilter->getCategoryId=".$this->ocfilter->getCategoryId();
	//$this->ocfilter->getCategoryId() = 59;
	...
	*/
    if ($this->config->get('module_ocfilter_show_price') && $this->ocfilter->getMinPrice() < $this->ocfilter->getMaxPrice() - 1) {
      $data['show_price'] = 1;
	  $data['filter_category_id'] = $this->ocfilter->getFilterCategoryId();
	...
	//$data['ocf_id']=$this->request->get['ocfilter_page_id'];
	//echo $data['ocf_id'];

	$data['ocf_id'] = $this->ocfilter->getPageInfo();
	$array_filter = $this->ocfilter->getPageInfo();
	$key = "video";
	/*
	if (isset($array_filter[$key])) {
		echo $array_filter[$key]; // Output: 25
	} else {
		echo "Key is not set";
	}*/
	...
	/* тут добавляються виробники по айді категорії */
	...
	if ((int)$this->ocfilter->getCategoryId() == (int)$ocdepartment->getCatNovinkiId())
		if ($this->session->data['language'] == 'uk-ua')
			$data['link']                 = HTTPS_SERVER.'uk/novinki';
		else
			$data['link']                 = HTTPS_SERVER.'novinki';
	else
		$data['link']                 = str_replace('&amp;', '&', $this->ocfilter->link('','',false));

	if (isset($this->request->get['filter_category_id']))
		$data['isCategoryFilterReset']                 = true;
	else
		$data['isCategoryFilterReset']                 = false;
	/* / fixed darkedhart */
  ...
  public function callback() {
	...
	if ($this->ocfilter->getCategoryId() == '777'){
		$filter_data = array(
	...
	if ($this->ocfilter->getCategoryId() == '777'){
		...
		$total_products = $this->model_extension_module_ocdepartment->getTotalProductSpecials($filter_data);
	...
	/* fixed darkedhart */
	//print_r("total_products=".$total_products."<br>");
	...
	$json['category_id'] = (int)$this->ocfilter->getCategoryId();
	$json['filter_category_id'] = (int)$this->request->get['filter_category_id'];
	
	require_once DIR_APPLICATION . 'controller/extension/module/ocdepartment.php';
	$ocdepartment = new ControllerExtensionModuleOCDepartment($this->registry);
	if ((int)$this->ocfilter->getCategoryId() == (int)$ocdepartment->getCatNovinkiId() && isset($this->request->get['filter_category_id'])){
		if ($this->session->data['language'] == 'uk-ua')
			$json['href'] = HTTPS_SERVER. 'uk/novinki?' . str_replace('&amp;', '&', 'filter_ocfilter='.$this->ocfilter->getParams()).'&filter_category_id='.$this->request->get['filter_category_id'];
		else
			$json['href'] = HTTPS_SERVER. 'novinki?' . str_replace('&amp;', '&', 'filter_ocfilter='.$this->ocfilter->getParams()).'&filter_category_id='.$this->request->get['filter_category_id'];
	}
	elseif ($this->ocfilter->getCategoryId() == '777'){
		
		if (isset($this->request->get['filter_category_id']) && (int)$this->request->get['filter_category_id'] !== 0){
			if ($this->session->data['language'] == 'uk-ua')
				$json['href'] = HTTPS_SERVER. 'uk/special?' . str_replace('&amp;', '&', 'filter_ocfilter='.$this->ocfilter->getParams()).'&filter_category_id='.$this->request->get['filter_category_id'];
			else
				$json['href'] = HTTPS_SERVER. 'special?' . str_replace('&amp;', '&', 'filter_ocfilter='.$this->ocfilter->getParams()).'&filter_category_id='.$this->request->get['filter_category_id'];
		}
		else
