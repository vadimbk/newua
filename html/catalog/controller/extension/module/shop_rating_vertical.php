...
class ControllerExtensionModuleShopRatingVertical extends Controller {
    ...
    public function index() {
        ...
        $data['show_rating'] = $this->config->get('module_shop_rating_vertical_show_rating');
        $data['text_summary'] = $this->language->get('text_summary');
        $data['text_count'] = $this->language->get('text_count');

        $ratings = $this->model_catalog_shop_rating->getLastStoreRatings($this->config->get('module_shop_rating_vertical_count'));
        $data['ratings'] =array();
