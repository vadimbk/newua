...
class ModelOCTemplatesBlogOCTBlogArticle extends Model {
	...
	public function getArticleAuthor($blogarticle_id) {
		$query = $this->db->query("
			SELECT DISTINCT
			aud.name AS author,
			aud.blogauthor_id AS blogauthor_id
			FROM
				" . DB_PREFIX . "oct_blogarticle a
				LEFT JOIN	
					" . DB_PREFIX . "oct_blogauthor_description aud
					ON (a.blogauthor_id = aud.blogauthor_id)
			WHERE
				a.blogarticle_id = '" . (int)$blogarticle_id . "'
				AND aud.language_id = '" . (int)$this->config->get('config_language_id') . "'
		");
	...
	public function getAuthorSeoUrls($blogauthor_id) {
		...
		AND 
		query = 'blogauthor_id=" . (int)$blogauthor_id . "'
		");
