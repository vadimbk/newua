...
class ModelExtensionModuleOCFilter extends Model {
  ...
  public function getOCFilterOptionsByCategoryId($category_id) {
	...
	$cache_key = 'ocfilter.option.' . (int)$category_id . '.' . $filter_category_id . '.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('module_ocfilter_sub_category');
  ...
  public function getManufacturersByCategoryId($category_id) {
	if ($category_id == 777){
		...
		$cache_key = 'ocfilter.special.manufacturers.' . (int)$category_id . '.' . (int)$filter_category_id . '.' . (int)$this->config->get('config_store_id') . '.' . (int)$this->config->get('module_ocfilter_sub_category');
		...
		// Запрос на выборку акционных товаров
			$product_sql = "SELECT DISTINCT ps.product_id 
			...
			// Запрос на выборку производителей
			$sql = "SELECT m.manufacturer_id AS value_id, m.name, 'm' AS option_id 
			...
			// Включаем подзапрос на акционные товары
			$sql .= " INNER JOIN (" . $product_sql . ") psq ON p.product_id = psq.product_id";
			
			if ($this->config->get('module_ocfilter_sub_category') && $filter_category_id !== 0) {
			...
			if ($filter_category_id !== 0){
				if ($this->config->get('module_ocfilter_sub_category')) {
					$sql .= " AND cp.path_id = '" . (int)$filter_category_id . "'";
				} else {
					$sql .= " AND p2c.category_id = '" . (int)$filter_category_id . "'";
				}
			}
		...
		if (isset($this->request->get['filter_category_id']))
			$filter_category_id = (int)$this->request->get['filter_category_id'];
		$distinct = "";
		if ($filter_category_id !== 0 && $category_id == (int)$ocdepartment->getCatNovinkiId())
			$distinct = " DISTINCT ";
		/* fixed darkedhart */
		
		$cache_key = 'ocfilter.manufacturer.' . (int)$category_id . '.' . $filter_category_id . '.' . (int)$this->config->get('config_store_id') . '.' . (int)$this->config->get('module_ocfilter_sub_category');
		...
		if ($this->config->get('module_ocfilter_sub_category')) {
			if ($filter_category_id !== 0 && $category_id == (int)$ocdepartment->getCatNovinkiId())
				$sql .= " AND cp.path_id IN ('" . (int)$category_id . "', '" . (int)$filter_category_id . "')";
			else
				$sql .= " AND cp.path_id = '" . (int)$category_id . "'";
		} else {
			if ($filter_category_id !== 0 && $category_id == (int)$ocdepartment->getCatNovinkiId())
				$sql .= " AND p2c.category_id IN ('" . (int)$category_id . "', '" . (int)$filter_category_id . "')";
			else
		...
		if ($filter_category_id !== 0 && $category_id == (int)$ocdepartment->getCatNovinkiId()){
			if ($this->config->get('module_ocfilter_sub_category')) 
				$sql2 .= ", p.product_id HAVING COUNT(DISTINCT cp.path_id) = 2 ";
			else
				$sql2 .= ", p.product_id HAVING COUNT(DISTINCT p2c.category_id) = 2 ";
		}
		/* / fixed darkedhart */
		$sql .= " GROUP BY m.manufacturer_id". $sql2 . "ORDER BY m.name ASC";
  ...
  public function getProductPrices($data) {
	...
	if (isset($this->request->get['filter_category_id']))
		...
		$cache_key = 'ocfilter.price.' . $filter_category_id . '.' .(int)$this->config->get('module_ocfilter_consider_discount') . '.' . (int)$this->config->get('module_ocfilter_consider_discount') . '.' . (int)$this->config->get('module_ocfilter_consider_option') . '.' . (int)$this->config->get('module_ocfilter_sub_category') . '.' . md5(serialize($data));
	...
	/* fixed */
	$product_sql_specials = "";
	 if ((int)$data['filter_category_id'] == 777){
		 // Запрос на выборку акционных товаров
		$product_sql_specials = "SELECT DISTINCT ps.product_id 
		FROM " . DB_PREFIX . "product_special ps 
		LEFT JOIN " . DB_PREFIX . "product p ON (ps.product_id = p.product_id) 
		LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) 
		LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) 
    ...
    $sql = "SELECT MIN(p.price) AS `min`, MAX(p.price) AS `max`, COUNT(DISTINCT p.product_id) AS pcount FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (p.product_id = p2c.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id)";
	
	
	
	if ((int)$data['filter_category_id'] == 777){
		$sql .= " INNER JOIN (" . $product_sql_specials . ") psq ON p.product_id = psq.product_id";
	...
	require_once DIR_APPLICATION . 'controller/extension/module/ocdepartment.php';
	$ocdepartment = new ControllerExtensionModuleOCDepartment($this->registry);
	if (isset($data['filter_category_id']) && isset($data['filter_category_id2']) && (int)$data['filter_category_id'] == (int)$ocdepartment->getCatNovinkiId()){
        ...
        WHERE 
            category_id IN (".(int)$data['filter_category_id'].", ".(int)$data['filter_category_id2'].")
        GROUP BY 
            product_id
        HAVING 
            COUNT(DISTINCT category_id) = 2) ";
		
	}elseif ((int)$data['filter_category_id'] == 777){
		if ($filter_category_id !== 0){
			if ($this->config->get('module_ocfilter_sub_category')) {
				$sql .= " AND cp.path_id = '" . (int)$filter_category_id . "'";
			} else {
  ...
  public function getSearchSQL($params = array()) {
	...
	public function getCounters($data = array()) {
		...
		$cache_key = 'product.ocfilter.counter.' . '.' . $filter_category_id . '.' . (int)$this->config->get('module_ocfilter_sub_category') . '.' . md5(serialize($data));
  ...
  private function getManufacturersCounterSQL($data) {
	...
	// Создайте объект контроллера
	$ocdepartment = new ControllerExtensionModuleOCDepartment($this->registry);
	if (isset($data['filter_category_id2']) && (int)$data['filter_category_id'] == (int)$ocdepartment->getCatNovinkiId()){
		$sql = "SELECT 
			p.manufacturer_id AS value_id, 
			'm' AS option_id, 
		...
		WHERE 
			...
			AND p.product_id IN (
				...
				WHERE category_id IN (".$data['filter_category_id'].", ".$data['filter_category_id2'].")
				GROUP BY product_id
