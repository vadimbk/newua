...
foreach ($coupon_manufacturer_query->rows as $manufacturer) {
	$coupon_manufacturer_data[] = (int)$manufacturer['manufacturer_id'];
}
...
if ($coupon_product_data || $coupon_category_data || $coupon_manufacturer_data) {
	foreach ($this->cart->getProducts() as $product) {
		...
		// Categories
		if (!$matched && $coupon_category_data) {
			foreach ($coupon_category_data as $category_id) {
				$coupon_category_check = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "product_to_category` WHERE product_id = '" . (int)$product['product_id'] . "' AND category_id = '" . (int)$category_id . "'");

				if ($coupon_category_check->row['total']) {
					$matched = true;
					break;
				}
			}
		...
		// Manufacturers
		if (!$matched && $coupon_manufacturer_data) {
			$product_manufacturer_query = $this->db->query("SELECT manufacturer_id FROM `" . DB_PREFIX . "product` WHERE product_id = '" . (int)$product['product_id'] . "'");

			if ($product_manufacturer_query->num_rows && in_array((int)$product_manufacturer_query->row['manufacturer_id'], $coupon_manufacturer_data)) {
				$matched = true;
			}
		}
