				...
				foreach ($this->langmark_settings['multi'] as $multi_name => $multi) {
					if (isset($this->languages_all[$multi['language_id']]['code'])) {
						...
						if ((isset($multi['main_prefix_status']) && $multi['main_prefix_status'] == 1) && (trim($route) == '' || utf8_strpos(utf8_strtolower($route), 'route=common/home') !== false) || ($multi['main_prefix_url'] != '' && $route == $multi['main_prefix_url'])) {
							...
							// staging-host fix: if request host suffix-matches prefix host, treat staging as same domain
							$lm_prefix_eff = $multi['prefix'];
							$lm_slash = strpos($lm_prefix_eff, '/');
							if ($lm_slash !== false) {
								$lm_pref_host = substr($lm_prefix_eff, 0, $lm_slash);
								$lm_req_host  = isset($this->request->server['HTTP_HOST']) ? preg_replace('/:\d+$/', '', $this->request->server['HTTP_HOST']) : '';
								if ($lm_pref_host !== '' && $lm_req_host !== '' && ($lm_req_host === $lm_pref_host || substr($lm_req_host, -strlen('.' . $lm_pref_host)) === '.' . $lm_pref_host)) {
									$lm_prefix_eff = $lm_req_host . substr($lm_prefix_eff, $lm_slash);
								}
							}
							$prefix_url = $full_url_data['scheme'] . '://' . $lm_prefix_eff . $multi['main_prefix_url'];
