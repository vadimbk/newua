...
final class OCFilter {
	...
	public function __construct($registry) {
        ...
        if ((isset($this->request->get['_route_'])) && $this->request->get['_route_'] == 'special'){
			//$filter_ocfilter_params = 'm:44';
			$this->path = 777;
			
			if (isset($this->request->get['filter_category_id']))
				$this->filter_category_id = $this->request->get['filter_category_id'];
		...
		if (isset($this->request->get['filter_category_id'])){
			$filter_data = array(
				...
				'filter_category_id2' => $this->request->get['filter_category_id'],
        ...
        if ($this->getParams()) {
			...
			foreach ($this->options_get as $option_id => $values) {
			    if ($option_id=='m'){
					$manufac_count += count($values);
			    }
			}

            if (!$this->getPageInfo() && (($values_count >= 2 && !isset($this->options_get['m'])) || ($values_count >2 && isset($this->options_get['m'])) || $manufac_count >= 2) ) {
                $this->document->setNoindex(true);
            }

            if (isset($this->options_get['p'])) {
                $this->document->setNoindex(true);
            }
            ...
            if ($option['type'] != 'slide' && $option['type'] != 'slide_dual') {
                foreach ($option['values'] as $value) {
                    ...
                    if ($count || !$this->config->get('module_ocfilter_hide_empty_values')) {
						...
						$filter_category_id = "";
						if (isset($this->request->get['filter_category_id']))
							$filter_category_id = "filter_category_id=".$this->request->get['filter_category_id']."&";
                        ...
                        if ($this_value) {
							...
							if($route == 'special'){
								...
								$m_link = "";
								$p_link = "";
								foreach($paramsExploded as $filter){
								...
								if ($data['current_lang']=='ru-ru')
									{$href = '/' . $route.'?'.$filter_category_id.'filter_ocfilter='.$m_link.(($p_link !== "")?";".$p_link:$p_link);}
								else{$href = '/uk/' . $route.'?'.$filter_category_id.'filter_ocfilter='.$m_link.(($p_link !== "")?";".$p_link:$p_link);}
							}
							elseif ((int)$this->category_id == (int)$ocdepartment->getCatNovinkiId()){
								/* fixed by darkedhart */
								...
								if ($data['current_lang']=='ru-ru')
								{$href = '/' . $route . $this->model_catalog_category->getFiltersSlug($params);}
								else{$href = '/uk/' . $route . $this->model_catalog_category->getFiltersSlug($params);	}
							}
                        }
...
//                                    $counter++;
                        ...
                        $manufacturer = false;
                        $manufacturer_nofollow = false;
                        if ($option['option_id'] == 'm') {
                            $manufacturer = true;
                            if(!empty($this->request->get['filter_ocfilter'])
                                && strpos($this->request->get['filter_ocfilter'], 'm:') !== false) {
                                $manufacturer_nofollow = true;
                            } else {
                                if (count($paramsExploded) > 2 ) {
                                    $manufacturer_nofollow = true;
                                }
                            }
                        }
                        ...
                        $values[] = array(
                            ...
                            'manufacturer_nofollow' => $manufacturer_nofollow,
	                    );
					}
                }
    ...
    public function getSelectedOptions() {
		...
		foreach ($category_options as $option) {
			...
			foreach ($option['values'] as $value) {
				...
				/* fixed darkedhart */
				$arr_path = explode('_',$this->getPath());
				if(count($arr_path) == 2 && $arr_path[0] == $arr_path[1])
					$this->path = $arr_path[1];
				/* /fixed darkedhart */
			    $values[] = array(
                    'name' => $name,
    ...
    public function link($filter_ocfilter = '', $filter_category_id = '', $isFilterCategoryId = true) {
		require_once DIR_APPLICATION . 'controller/extension/module/ocdepartment.php';
		// Создайте объект контроллера
		$ocdepartment = new ControllerExtensionModuleOCDepartment($this->registry);
        ...
        $url = '';
		/* fixed darkedhart */
        if ($this->getPath() && (string)$this->getPath() !== '777' && (int)$this->getPath() !== (int)$ocdepartment->getCatNovinkiId()) {
            $url .= '&path=' . (string)$this->getPath();
        }
		/* для новинок */
		if ($filter_category_id || ((int)$this->getPath() == (int)$ocdepartment->getCatNovinkiId() && isset($this->request->get['filter_category_id']))) {
            $url .= '&filter_category_id=' . (string)$this->request->get['filter_category_id'];
        }
		/* для special */
		if ($this->getFilterCategoryId() && $isFilterCategoryId) {
            $url .= '&filter_category_id=' . (string)$this->getFilterCategoryId();
        }
