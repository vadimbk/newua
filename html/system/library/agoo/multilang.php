<?php
// DISABLED — class kept as a parked artifact, not part of any active flow.
//
// The live URL machinery lives in system/library/seolang/seolang.php and
// catalog/controller/seolang/mova/mova.php (structurally identical to this
// file). The "// From /system/library/agoo/multilang.php" markers inside
// catalog/controller/agoo/langmark/langmark.php and
// catalog/controller/record/langmark.php point to code that was copied out
// of here. Nothing instantiates agooMultilang anywhere in the codebase: no
// `new`, no `$load->library('agoo/multilang')`, no controller_agoo_multilang
// registration, no class_exists/instanceof check. Empirically confirmed via
// an opcache audit (zero matches for /agoo/* after exercising a wide spread
// of catalog URLs in both Russian and Ukrainian).
//
// To revive: delete this comment block and the `return;` below.
return;
...
class agooMultilang extends Controller
{
	...
	public function __construct($registry) {
        ...
        if (!isset($_COOKIE['language']) && !isset($_COOKIE['langmark_nocookie'])) {
        	setcookie('langmark_nocookie', true, time() + 60 * 60 * 24 * 30, '/', $this->request->server['HTTP_HOST']);
        } else {
        	if (isset($_COOKIE['langmark_nocookie'])) {
        		unset($_COOKIE['langmark_nocookie']);
        		setcookie('langmark_nocookie', null, -1, '/', $this->request->server['HTTP_HOST']);
        	}
        }

		if (isset($_COOKIE['language']) && $_COOKIE['language'] != $this->config->get('config_language') && isset($this->langcode_all[$_COOKIE['language']]['language_id'])) {
