...
//Функции экспорта
    ...
    function loaded_export_setting(feed){
      ...
      $('#input-products').autocomplete({
        'source': function(request, response) {
          $.ajax({
            url: 'index.php?route=' + unixml_path + '/exportAutocompleteProduct' + unixml_token + '&filter_name=' +  encodeURIComponent(request) + '&feed=' + $('input[name="feed"]').val(),
            dataType: 'json',
  ...
  //export_trash - отправка фида в корзину
    $(document).on('click', '.export_trash', function() {
      feed = $(this).data('feed');
      $.ajax({
        url: 'index.php?route=' + unixml_path + '/exportFeedToTrash' + unixml_token + '&feed=' +  feed,
        dataType: 'json',
        success: function(json) {
          update_export_list();
        }
  ...
  //trash_list_toggle - скрытие и раскрытие корзины
    $(document).on('click', '#trash_list_toggle', function() {
      ...
      $.ajax({url: 'index.php?route=' + unixml_path + '/exportTrashToggle' + unixml_token + '&status=' +  status});
    });
  //trash_list_toggle
  ...
  //export_to_list - убрать с корзины обратно в список
    $(document).on('click', '.export_to_list', function() {
      feed = $(this).data('feed');
      $.ajax({
        url: 'index.php?route=' + unixml_path + '/exportFeedToList' + unixml_token + '&feed=' +  feed,
        dataType: 'json',
        success: function(json) {
          update_export_list();
        }
  ...
  //getXMLFileInfo - функция просмотра информации о файле
    function getXMLFileInfo(){
      $.ajax({
        url: 'index.php?route=' + unixml_path + '/exportGetXMLFileInfo' + unixml_token + '&file=' +  $('#unixml_xml_name').val(),
        dataType: 'json',
        success: function(json) {
...
//Функции сервиса
        ...
        success: function(json) {
          ...
          if(json.length !== 0){
            ...
            $.each(json, function (index, value) {
              ...
              table += '<td>';
              $.each(value['keywords'], function (seo_index, keyword_value) {
                ...
                table += '<input data-id="' + keyword_value['seo_url_id'] + '" type="text" class="form-control update_input_' + keyword_value['seo_url_id'] + '" value="' + keyword_value['keyword'] + '">';
                table += '<span class="input-group-addon save_url_item" data-seo_index="' + keyword_value['seo_url_id'] + '"><i class="fa fa-floppy-o" aria-hidden="true"></i></span></div>';
              });
              table += '</td>';
              table += '</tr>';
            });
  ...
  //save_url_item
    $(document).on('click', '.save_url_item', function(e){
      seo_index = $(this).data('seo_index');
      seo_keyword = $(this).parent().find('input').val();

      $('.update_input_' + seo_index).addClass('url_saved');

      $.ajax({
        url: 'index.php?route=' + unixml_path + '/serviceUrlSave' + unixml_token + '',
        dataType: 'json',
        data: {'seo_index': seo_index, 'seo_keyword': seo_keyword},
        method: 'post',
        success: function(json) {
          setTimeout(function(){
            $('.update_input_' + seo_index).removeClass('url_saved');
          }, 1000);
        }
      });
    });
