var ocfilter = {
	...
	**/
	productForm: {
		category_id: null,
		product_id: null,
    categorySelector: 'select[name="main_category_id"], input[type="hidden"][name="product_category[]"], input[type="checkbox"][name="product_category[]"]:checked',
		init: function() {
      let that = this;
      ...
      let length = $(that.categorySelector).length;

	  	setInterval(function() {
				let _length = $(that.categorySelector).length;
      ...
      $(that.categorySelector).each(function(i) {
        if (-1 === that.category_id.indexOf($(this).val())) {
          that.category_id.push($(this).val());
        }
      });
