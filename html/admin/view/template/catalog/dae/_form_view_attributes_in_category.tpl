...
<script>
   document.addEventListener(DAE_EVENT_SELECT_SINGLE_CATEGORY, function(e) {
	$.ajax({
            url: JS_URL_GET_VIEW_ATTRIBUTES_BY_CATEGORY,
            type: "POST",
            ...
            success: function (json) {
                ...
                if(json.hasOwnProperty('settings') && ('template' in json.settings)){
                //вывод настроек(шаблона)
                    $('#template').val(json.settings['template']);
                }
            }
        });
    });
     ...
     */
     $('#save-form').click(function () {
        ...
        var form = $('#dae-form');
        $.ajax({
            url: JS_URL_SAVE_FORM_VIEW_ATTRIBUTES_IN_CATEGORY,
            type: "POST",
            data: form.serialize(),
            dataType: 'json',
            success: function (json) {
